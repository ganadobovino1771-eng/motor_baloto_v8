...
# ==========================================
acum_baloto   = 36400   # Millones
acum_revancha = 2200   # Millones
...
def ejecutar_sistema_profesional():
    ...
    print("─"*45)
    confirmacion = input("  ¿El CSV tiene el resultado más reciente? (s/n): ").strip().lower()
    if confirmacion != 's':
        print("\n  ⚠ Actualiza el archivo Baloto_Historico_Perfecto.csv")
        print("  con el resultado del último sorteo y vuelve a ejecutar.")
        print("─"*45)
        return
    ...
    if os.path.exists(archivo_log):
        ...
        for i, row in df_log.iterrows():
            if row['Aciertos_Post'] == 'Pendiente':
                ...
                sb_ant     = int(row['SB'])
                aciertos   = len(np.intersect1d(jugada_ant, num_espejo))
                sb_acierto = 1 if sb_ant == sb_espejo else 0
    ...
    print(f"        (Versión 8.6 — Blindaje y Control de Calidad)")
    print("█"*45)
    ...
    print("─"*45)
    print("  AUDITORÍA INTELIGENTE v8.6")
    print("─"*45)
